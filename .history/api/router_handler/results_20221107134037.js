        ...
        db.query(sqlStr, i, (err, results) => {
            ...
            if (results.length > 0) {
                const sql = 'update testt set number=number+? where id=?'
                    // console.log(i)
                    // let k = 1
                db.query(sql, [req.body.salty, 1], (err, results) => {
                    if (err) return res.cc(err)
        ...
        db.query(sqlStr, i, (err, results) => {
            ...
            if (results.length > 0) {
                const sql = 'update testt set number=number+? where id=?'
                    // console.log(i)
                    // let k = 1
                db.query(sql, [req.body.tomato_flavor, 2], (err, results) => {
                    if (err) return res.cc(err)
                        // SQL 语句执行成功，但是影响行数不等于 1
                    if (results.affectedRows !== 1) return res.cc('投票失败！')
                        // 投票成功
        ...
        db.query(sqlStr, i, (err, results) => {
            ...
            if (results.length > 0) {
                const sql = 'update testt set number=number+? where id=?'
                    // console.log(i)
                    // let k = 1
                db.query(sql, [req.body.raw_starchFlavor, 3], (err, results) => {
                    if (err) return res.cc(err)
                        // SQL 语句执行成功，但是影响行数不等于 1
                    if (results.affectedRows !== 1) return res.cc('投票失败！')
                        // 投票成功
        ...
        db.query(sqlStr, i, (err, results) => {
            ...
            if (results.length > 0) {
                const sql = 'update testt set number=number+? where id=?'
                    // console.log(i)
                    // let k = 1
                db.query(sql, [req.body.brittleness, 4], (err, results) => {
                    if (err) return res.cc(err)
                        // SQL 语句执行成功，但是影响行数不等于 1
                    if (results.affectedRows !== 1) return res.cc('投票失败！')
                        // 投票成功
        ...
        db.query(sqlStr, i, (err, results) => {
            ...
            if (results.length > 0) {
                const sql = 'update testt set number=number+? where id=?'
                    // console.log(i)
                    // let k = 1
                db.query(sql, [req.body.sweet_taste, 5], (err, results) => {
                    if (err) return res.cc(err)
                        // SQL 语句执行成功，但是影响行数不等于 1
                    if (results.affectedRows !== 1) return res.cc('投票失败！')
                        // 投票成功
