...
exports.postResults = (req, res) => {
        ...
        console.log(req.body.salty)
        const sqlStr = 'select * from testt where test="salty" '
        db.query(sqlStr, (err, results) => {
            if (err) return res.cc(err)
            if (results.length > 0) {
                const sql = 'update testt set number=number+? where test="salty"'
                db.query(sql, req.body.salty, (err, results) => {
                    if (err) return res.cc(err)
